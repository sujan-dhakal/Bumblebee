...
#include "ui_singledaydata.h"

Singledaydata::Singledaydata(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Singledaydata)
{
    ui->setupUi(this);
    minX = 0;
    maxX = 5;
...
void Singledaydata::on_move_left_clicked(){
    minX = minX - 0.05;
    maxX = maxX - 0.05;
    ui->nest->xAxis->setRange(minX, maxX);
    ui->nest->replot();
...
void Singledaydata::on_move_right_clicked(){
    minX = minX + 0.05;
    maxX = maxX + 0.05;
    ui->nest->xAxis->setRange(minX, maxX);
    ui->nest->replot();
...
void Singledaydata::fill_events() {
    Parse p;
